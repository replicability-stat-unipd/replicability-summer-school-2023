...
To compare results from different studies, we should use a common metrics. Frequently meta-analysis use the so called *standardized* effect size. For example the Pearson correlation or the Cohen's $d$.
...
The advantage of standandardized effect size is that regardless the original variable, the intepretation and the scale is the same. For example the pearson correlation ranges between -1 and 1 and the Cohen's $d$ between $- \inf$ and $\inf$ and is intepreted as how many standard deviations the two groups differs.
...
## Effect size Standard Error {#sec-effsize-se}

Crucially, we can calculate also the standard error of each effect size. The standard error tell us how precise is the estimate.

::: {.panel-tabset}

### Formula

For example, he standard error of the Cohen's $d$ has several estimator. Just as example:
$$
SE_d = \frac{n_1 + n_2}{n_1 n_2} + \frac{d^2}{2(n_1 + n_2)}
$$
...
Meta-analysis notation is a little bit inconsistent in textbooks and papers. We define here some rules to simplify the work^[The proposed notation could not be the best from the formal point of view]
...
Let's imagine to have $k = 10$ studies:
...
We could say that the average effect is roughly ~0.5 and there is not a lot of variability around the average.
...
$$
\overline y = \frac{\sum_{i = 1}^k y}{k}$T$\
$$
...
The simple average is a good statistics. But some studies are clearly more precise (narrower confidence intervals) than others.
...
We can compute a weighted average where each study is weighted ($w_i$) by the inverse of the variance. This is called inverse-variance weighting. Clarly a weighted average were all weights are the same reduced to a simple aritmetic average.
...
Compared to the unweighted version (`r ybar`), given that weights are non homogeneous the values are different.
...
Where $\sigma^2_i$ is the vector of sampling variabilities of $k$ studies. Clearly this is a model with heterogeneous variances. In meta-analysis the sampling variances (e.g., calculated as in Slide [-@sec-effsize-se])
...
Given that we extended the EE model equation. Also the estimation of the average effect need to be extended. Basically the RE is still a weigthed average but weigths need to include also $\tau^2$.
...
For the simulations, I'm sampling from effect size and variance sampling distributions.

::: {.panel-tabset}

### Equations

The unstandardized effect size is a mean difference. The sampling distribution is:

$$
y_i \sim \mathcal{N}(\Delta, \frac{1}{n_1} + \frac{1}{n_2})
$$

Where ES is the population level effect size and $n_{1,2}$ are the sample sizes of the two studies. The sampling variances are generated from a $\chi^2$ distribution:
$$
...
There are several estimators for $\tau^2$ [see @Viechtbauer2005-zt; @Veroniki2016-zs]
...
hist(unlist(res2_tau2_0), probability = TRUE, main = latex2exp::TeX("$\\tau^2 = 0$"), ylim = c(0, 2),
     xlab = latex2exp::TeX("$y_i - \\hat{\\theta_i}$"))
...
- The `size of the square` is the **weigth** of that study ($w_i = 1/\sigma_i^2$)
...
The PB is one of the most problematic aspects of meta-analysis. Essentially the probability of publishing a paper (and thus including into the meta-analysis) is not the same regardless the result.
...
Clearly we cannot solve the PB using statistical tools. The PB is a problem of the publishing process and cannot be completely removed from available literature.

- pre-registrations, multi-lab studies, etc. can completely solve the problem populating the literature with unbiased studies (at least from the publishing process point of view)
- there are statistical tools to detect, estimate and correct for the publication bias. Unfortunately they are strongly affected by the presence of heterogeneity, the number of studies and other factors.
...
%%| fig-width: 6.5
flowchart TD
    A[Publication Bias] --> B[Visual Inspection]
    A[Publication Bias] --> L[Robustness]
    A[Publication Bias] --> C[Detection]
    A[Publication Bias] --> D[Correction]
    L[Robustness] --> M[Fail-safe N]
    B[Visual Inspection] --> E[Funnel Plot]
    C[Detection] --> G[Regression Methods]
    D[Correction] --> H[Trim and Fill]
    D[Correction] --> I[Selection Models]
```

## Publication Bias (PB) - Funnel Plot

The first tool is called funnel plot. This is a visual tool to check the presence of publication bias. If meta-analysis assumptions are respected, and there is no publication bias:
...
- is not actually *detecting* the PB but suggesting the required publication bias size to remove the effect. A very large N suggest that even if present, it's unlikely the results could be completely changed by actually reporting null studies
...
The trim-and-fille estimate that `r taf$k0` are missing. The new effect size after including the studies is reduced and closer to the simulated value.
...
And adding the weigth function:
...
For example, Citkowicz and Vevea [-@Citkowicz2017-ox] proposed a model using the beta weight function^[[https://www.youtube.com/watch?v=ucmOCuyCk-c](https://www.youtube.com/watch?v=ucmOCuyCk-c)] 
...
We will analyze the data provided by [https://www.metapsy.org/](https://www.metapsy.org/). In particular the dataset about the efficacy of psychoterapy on depression using randomized-controlled trials ([https://www.metapsy.org/database/depression-psychotherapy](https://www.metapsy.org/database/depression-psychotherapy)).

```{r}
#| echo: false
knitr::include_url("https://docs.metapsy.org/")
```

## Dataset

The dataset for the exercises is `depression.rds`. The dataset has been minimally pre-processed to remove extra columns and converted to the `rds` format.

In particular:

- studies using *cognitive-behavioral* therapy
- studies with controls in waiting lists
- studies with therapy delivered individually or in groups
...
- `study`: identifier for each included study
- `age_group`: the age group of individual in the experimental group (`adul` = adults, `yadul` = young adults, `old` = old)
- `mean_age`: the average age of the experimental group
- `country`: country where the study was conducted
- `format`: the therapy format (`grp` = group, `ind` = individual)
- `yi`: the effect size in the form of Hedges's $g$ [see @Hedges1989-ip, Cohen's $d$ corrected for small-sample bias]. Positive values means that the treatment is effective.
- `sei`: standard error of the Hedges's $g$

## Pre-processing

> **Crucially**, some papers have multiple studies. This data structure should be handled using a multilevel model. For the sake of simplicity we sampled randomly a study from multiple studies.

These are the pre-processing steps:
```{r}
#| output: asis
#| echo: false
filor::show_file(here("02-meta-analysis/scripts/depression-pre-processing.R"), how = "code")
```

## Steps

> **Important**, the dataset is real but the preprocessing steps and the models are examples without a clinical meaning. We are surely missing important predictors and/or analysis steps thus take the results only for educational purposes.
...
5. Analize the publication bias using the Egger regression and comment the result

## Advanced Steps

We did not introduced **meta-regressions** i.e. the possibility to include predictors in a meta-analysis model. The implementation is quite straightforward. E.g. see [https://bookdown.org/MathiasHarrer/Doing_Meta_Analysis_in_R/metareg.html](https://bookdown.org/MathiasHarrer/Doing_Meta_Analysis_in_R/metareg.html) and the `mods` argument of `metafor::rma.uni()` function [https://wviechtb.github.io/metafor/reference/rma.uni.html](https://wviechtb.github.io/metafor/reference/rma.uni.html).

- Try to compute a meta-regression model checking the impact of `format`
  - intepret the results
  - is the heterogeneity different from the model without predictors?

# Proposed solutions {.section}

{{< include 02-meta-analysis-solutions.qmd >}}
