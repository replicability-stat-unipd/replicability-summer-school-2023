...
@Spence2016-tz proposed to use the prediction interval to assess the range of effects that we could expect assuming that the original and the (hypothetical) replication study comes from the same population (i.e., successful replication).
...
If factors other than standard error influence the replication result, $\theta_{rep_0}$  is expected to be contained within the 95% prediction interval.
...
Let's make a simulated example:
